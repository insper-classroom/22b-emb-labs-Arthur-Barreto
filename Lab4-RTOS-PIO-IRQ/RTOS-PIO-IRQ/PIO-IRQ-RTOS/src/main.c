...
/* semafora para diminuir a frequencia do led */
...
void but_callback(void) {
    ...
    int value = 100;
    xQueueSendFromISR(xQueueLedFreq, &value, &xHigherPriorityTaskWoken);
...
static void task_led(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (xQueueReceive(xQueueMy, &msg, (TickType_t)0)) {
...
static void task_but(void *pvParameters) {
    ...
    for (;;) {
        ...
        /* usando somente fila 
            primeiro a fila do botão da placa
            depois a fila do botão da plaquinha oled
        */
        
        if (xQueueReceive(xQueueLedFreq, &value,(TickType_t)0)){
            /* atualiza delay_ticks */
            delayTicks += value;
            /* se dalay == 100, nao podemos diminuir */
            if (delayTicks <= 100) {
                delayTicks = 900;
            }

            /* agora envia para a task do led o valor de daly ticks */
            printf("Valor enviado de ticks = %d\n",delayTicks);
            xQueueSend(xQueueMy, &delayTicks, &xHigherPriorityTaskWoken);
        }
...
int main(void) {
    ...
    xQueueMy = xQueueCreate(32, sizeof(uint32_t));
